...
class TapePainter extends CustomPainter {
  ...
  void _drawLabel() {
    ...
    Rect labelTop = Rect.fromLTRB(label.left, label.top + label.height * 0.2,
        label.right, label.bottom - label.height * 0.1);
  ...
  void _drawTextLabel() {
    TextSpan span =
        TextSpan(style: TextStyle(color: Colors.black), text: title);
    ...
    final offset = Offset((size.width - textPainter.width) * 0.5,
  ...
  void _drawTapePin(Offset center, double angle) {
    _drawRotated(Offset(center.dx, center.dy), -angle, () {
      canvas.drawRect(
          Rect.fromLTWH(
              center.dx - 2, center.dy - holeRadius, 4, holeRadius / 4),
          paintObject);
    });
  }
  ...
  void _drawTapeReels() {
    Path lefTapRoll = Path()
      ..addOval(Rect.fromCircle(
          center: leftHolePosition, radius: holeRadius * (1 - progress) * 5));
    Path rightTapRoll = Path()
      ..addOval(Rect.fromCircle(
          center: rightHolePosition, radius: holeRadius * progress * 5));

    lefTapRoll = Path.combine(PathOperation.difference, lefTapRoll, leftHole);
    rightTapRoll =
        Path.combine(PathOperation.difference, rightTapRoll, rightHole);

    paintObject.color = Colors.black;
    canvas.drawPath(lefTapRoll, paintObject);
    canvas.drawPath(rightTapRoll, paintObject);
  }
